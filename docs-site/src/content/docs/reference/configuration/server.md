...
| `websockets?` | `boolean` | `false` | Advertise and admit the client-facing Responses WebSocket path. False keeps clients on HTTP/SSE; it does not disable an eligible canonical ChatGPT upstream WS optimization. Each upstream request uses a separate connection that closes when the response completes. Active exchanges remain tracked for cancellation and shutdown. HTTP input is not trimmed and previous-response IDs are not created. |
