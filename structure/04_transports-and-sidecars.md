...
Canonical upstream sockets close when each response completes, including repeated
requests within the same account, credential, thread and turn. Eligible active
exchanges remain tracked for cancellation and shutdown; completed sockets are not
retained and do not create idle-expiry timers. A busy key uses an independent
connection rather than interleaving requests.

Turn-state and turn-metadata headers are projected into their same-name per-frame
metadata slots; explicit body values win. Complete HTTP inputs are preserved and
no previous response id is invented. A fresh credential-dispatch guard runs before
every send. Per-exchange listeners, response/item correlation and metadata
ownership detach before release. Shutdown registration ends when the last tracked
exchange closes.
